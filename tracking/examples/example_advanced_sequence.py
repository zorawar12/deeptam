...
def track_rgbd_sequence(checkpoint, datadir, tracking_module_path, visualization):
    ...
    frame = sequence.get_dict(
        0, intrinsics, tracker.image_width, tracker.image_height)
    pose0_gt = frame['pose']
    tracker.clear()
    # this step can be left out if gt_poses is aligned such that it starts
    # from identity pose
    ...
    for frame_idx in range(sequence.get_sequence_length()):
        ...
        frame = sequence.get_dict(
            frame_idx, intrinsics, tracker.image_width, tracker.image_height)
        ...
        if visualization:
            update_visualization(axes, pr_poses, gt_poses, frame[
    ...
    print('Frame-to-keyframe odometry evaluation [RPE], translational RMSE: {}[m/s]'.format(
...
def main():
    ...
    ''')
    parser.add_argument(
        '--data_dir', help='set a sequence data directory (should be consistent with TUM RGBD-SLAM datasets), by default use sequence ../data/rgbd_dataset_freiburg1_desk', default=None)
    parser.add_argument(
        '--disable_vis', help='disable the frame-by-frame visualization for speed-up', action='store_true')
    ...
    checkpoint = os.path.join(
        examples_dir, '..', 'weights', 'deeptam_tracker_weights', 'snapshot-300000')
    if data_dir is None:
        data_dir = os.path.join(
            examples_dir, '..', 'data', 'rgbd_dataset_freiburg1_desk')
    tracking_module_path = os.path.join(
        examples_dir, '..', 'python/deeptam_tracker/models/networks.py')

    sequence, timestamps = track_rgbd_sequence(
        checkpoint, data_dir, tracking_module_path, visualization)

    pickle.dump(sequence, open('sequence.pkl', 'wb'))
    pickle.dump(timestamps, open('timestamps.pkl', 'wb'))
