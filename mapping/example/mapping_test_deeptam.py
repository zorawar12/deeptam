...
DepthMetrics = namedtuple(
    'DepthMetrics', ['l1_inverse', 'scale_invariant', 'abs_relative'])
import pickle
...
def create_cv_conf_from_sequence_py(depth_key,
    ...
    # computes pairwise cost volume with its conf between the keyframe and all
...
def mapping_with_pose(
    ...
    # fixed band module
    fb_depth_net = mapping_mod.MappingFBNetwork(
    ...
    # narrow band module
    nb_depth_net = mapping_mod.MappingNBNetwork(
        batch_size=1, width=width, height=height)
    ...
    nb_refine_depth_net = mapping_mod.MappingNBRefineNetwork(
        batch_size=1, width=width, height=height)
    ...
    cv_fb_generate_net = mapping_mod.CVGenerateNetwork(
        batch_size=1, width=width, height=height, depth_scale_array=np.linspace(0.01, 2.5, 32))
    ...
    cv_nb_generate_net = mapping_mod.CVGenerateNetwork(
        batch_size=1, width=width, height=height, depth_scale_array=np.linspace(0.8, 1.2, 32))
    ...
    for frame_id in range(1, sub_seq_py.seq_len):
        ...
        cv, depth_label_tensor = create_cv_conf_from_sequence_py(
            depth_init, sub_sub_seq_py, session, cv_fb_generate_net, cv_fb_generate_outputs)
        feed_dict = {
    ...
    for iteration in range(nb_iterations_num):
        ...
        cv, depth_label_tensor = create_cv_conf_from_sequence_py(
            depth_pr, sub_seq_py, session, cv_nb_generate_net, cv_nb_generate_outputs)
        feed_dict = {
        ...
        nb_refine_out = session.run(
            nb_refine_depth_outputs, feed_dict=feed_dict)
...
def main():
    ...
    mapping_module_path = os.path.join(
        examples_dir, '..', 'python/deeptam_mapper/models/networks.py')
